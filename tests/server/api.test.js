...
test('versioned style namespace serves the current shell stylesheet', async () => {
  const response = await fetch(baseUrl + '/styles/v410/56-profile.css');
  const source = await response.text();
  assert.equal(response.status, 200);
  ...
  const versionedRuntime = await fetch(baseUrl + '/scripts/v410/render.js');
  assert.equal(versionedRuntime.status, 200);
  assert.match(await versionedRuntime.text(), /renderProfileSummary/);

  const traversalAttempt = await fetch(baseUrl + '/scripts/v410/..%2Fserver.js');
