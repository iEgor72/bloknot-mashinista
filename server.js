...
const communitySectionResolver = require('./server/community-section-resolver');
const {
...
const CATALOG_ROOT_DIR = path.join(ROOT, 'assets', 'catalog');
const CATALOG_RAILWAYS_FILE = path.join(CATALOG_ROOT_DIR, 'railways.json');
const CATALOG_DEPOTS_FILE = path.join(CATALOG_ROOT_DIR, 'depots.json');
const TRACKER_SECTIONS_DIR = path.join(ROOT, 'assets', 'tracker', 'sections');
...
const communityProposalAttempts = new Map();
const communityReviewAttempts = new Map();
...
const COMMUNITY_KINDS = new Set(['document', 'speed', 'profile', 'object', 'geometry', 'section']);
const COMMUNITY_RISK_LEVELS = new Set(['normal', 'safety_restriction', 'safety_increase']);
const COMMUNITY_SCOPE_LEVELS = new Set(['network', 'railway', 'depot', 'service_arm', 'section']);
const COMMUNITY_MEMBERSHIP_LEVELS = new Set(['railway', 'depot', 'service_arm']);
const COMMUNITY_MEMBERSHIP_STATUSES = new Set(['selected', 'pending', 'verified', 'suspended']);
const COMMUNITY_MEMBERSHIP_ROLES = new Set(['member', 'reviewer', 'curator']);
const COMMUNITY_REVIEW_VERDICTS = new Set(['confirm', 'needs_fix', 'reject', 'abstain']);
const COMMUNITY_ELECTION_STATUSES = new Set(['draft', 'nominations', 'voting', 'closed', 'cancelled']);
const COMMUNITY_PROPOSAL_RATE_WINDOW_MS = 30 * 60 * 1000;
const COMMUNITY_PROPOSAL_RATE_MAX = 12;
const COMMUNITY_REVIEW_RATE_WINDOW_MS = 10 * 60 * 1000;
const COMMUNITY_REVIEW_RATE_MAX = 60;
const COMMUNITY_MAX_CHANGE_BYTES = 24 * 1024;
const COMMUNITY_MAX_EVIDENCE_BYTES = 12 * 1024;
const COMMUNITY_OBJECT_TYPES = new Map([
  ['station', { collection: 'stations', defaultName: 'Станция' }],
  ['signal_input', { collection: 'signals', defaultName: 'Входной' }],
  ['signal_output', { collection: 'signals', defaultName: 'Выходной' }],
  ['signal_passage', { collection: 'signals', defaultName: 'Проходной' }],
  ['sign_c', { collection: 'whistle_points', defaultName: 'С' }],
  ['whistle', { collection: 'whistle_points', defaultName: 'Подать свисток' }],
  ['ktsm', { collection: 'infrastructure', defaultName: 'КТСМ' }],
  ['brake_start', { collection: 'control_marks', defaultName: 'НТ' }],
  ['brake_end', { collection: 'control_marks', defaultName: 'КТ' }],
  ['neutral', { collection: 'control_marks', defaultName: 'ОМ' }],
  ['throttle', { collection: 'control_marks', defaultName: 'Позиция' }],
  ['connection', { collection: 'control_marks', defaultName: 'Соединение' }],
  ['brake_note', { collection: 'annotations', defaultName: 'Торможение' }],
  ['position_note', { collection: 'annotations', defaultName: 'Позиции' }],
  ['note', { collection: 'annotations', defaultName: 'Пометка' }],
]);
const COMMUNITY_SAFETY_OBJECT_TYPES = new Set([
  'station', 'signal_input', 'signal_output', 'signal_passage', 'sign_c', 'whistle', 'ktsm', 'brake_start', 'brake_end', 'neutral',
]);
const COMMUNITY_RESTRICTIVE_OBJECT_TYPES = new Set(['sign_c', 'whistle', 'brake_start']);

function readCommunityCatalog() {
  const railwaysDoc = readJsonFileForAdmin(CATALOG_RAILWAYS_FILE, { railways: [] });
  const depotsDoc = readJsonFileForAdmin(CATALOG_DEPOTS_FILE, { depots: [] });
  return {
    railways: Array.isArray(railwaysDoc && railwaysDoc.railways) ? railwaysDoc.railways : [],
    depots: Array.isArray(depotsDoc && depotsDoc.depots) ? depotsDoc.depots : [],
  };
}

function findCommunityRailway(catalog, railwayId) {
  const target = String(railwayId || '');
  return (catalog && Array.isArray(catalog.railways) ? catalog.railways : [])
    .find((item) => item && String(item.id || '') === target) || null;
}

function findCommunityDepot(catalog, depotId) {
  const target = String(depotId || '');
  return (catalog && Array.isArray(catalog.depots) ? catalog.depots : [])
    .find((item) => item && String(item.id || '') === target) || null;
}

function readCommunityDepotPack(depot) {
  const relativePath = depot && depot.pack_file ? String(depot.pack_file).replace(/\\/g, '/') : '';
  if (!relativePath || relativePath.includes('..') || relativePath.startsWith('/')) return null;
  const filePath = path.resolve(path.join(CATALOG_ROOT_DIR, relativePath));
  if (!filePath.startsWith(path.resolve(CATALOG_ROOT_DIR) + path.sep)) return null;
  return readJsonFileForAdmin(filePath, null);
}

function communityScopeKey(scope) {
  const item = scope || {};
  if (item.level === 'network') return 'network';
  if (item.level === 'railway') return `railway:${item.railwayId}`;
  if (item.level === 'depot') return String(item.depotId || '');
  if (item.level === 'service_arm') return `service_arm:${item.depotId}:${item.serviceArmId}`;
  if (item.level === 'section') return `section:${item.sectionId}`;
  return '';
}

function sanitizeCommunityJsonObject(value, maxBytes, label) {
  if (value == null) return {};
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error(`Invalid ${label}`);
  const serialized = JSON.stringify(value);
  if (Buffer.byteLength(serialized) > maxBytes) throw new Error(`${label} payload too large`);
  return JSON.parse(serialized);
}

function readCommunityTrackerSection(sectionId) {
  const id = String(sectionId || '');
  if (!/^[a-z0-9][a-z0-9-]{1,159}$/.test(id)) return null;
  const filePath = path.resolve(path.join(TRACKER_SECTIONS_DIR, `${id}.json`));
  if (!filePath.startsWith(path.resolve(TRACKER_SECTIONS_DIR) + path.sep)) return null;
  const section = readJsonFileForAdmin(filePath, null);
  return section && String(section.id || '') === id ? section : null;
}

function listPublishedCommunitySectionReleases(sectionId) {
  const scopeKey = `section:${String(sectionId || '')}`;
  return getStorage().listCommunityReleases(1000)
    .filter((release) => release.scopeLevel === 'section' && release.scopeKey === scopeKey && release.status === 'published');
}

function resolveEffectiveCommunitySection(sectionId) {
  const baseSection = readCommunityTrackerSection(sectionId);
  if (!baseSection) return null;
  return communitySectionResolver.resolveCommunitySection(baseSection, listPublishedCommunitySectionReleases(sectionId));
}

function communitySectionObjectCoordinate(item) {
...
function inferCommunitySectionObjectKind(collection, item) {
...
function listCommunitySectionObjects(section) {
  return communitySectionResolver.listSectionObjects(section).map((item) => {
    const definition = COMMUNITY_OBJECT_TYPES.get(item.kind) || COMMUNITY_OBJECT_TYPES.get('note');
    return {
      ...item,
      name: sanitizeProfileText(item.name, 80) || definition.defaultName,
    };
  });
}

function communityGeoDistanceMeters(latA, lonA, latB, lonB) {
  const toRad = (value) => Number(value) * Math.PI / 180;
  const dLat = toRad(Number(latB) - Number(latA));
  const dLon = toRad(Number(lonB) - Number(lonA));
  const first = toRad(latA);
  const second = toRad(latB);
  const haversine = Math.sin(dLat / 2) ** 2 + Math.cos(first) * Math.cos(second) * Math.sin(dLon / 2) ** 2;
  return 6371000 * 2 * Math.atan2(Math.sqrt(haversine), Math.sqrt(Math.max(0, 1 - haversine)));
}

function sanitizeCommunityVisualChange(kind, change, scope) {
  if (!change || change.editor !== 'visual-v1') return { change, baseVersion: '' };
  if (scope.level !== 'section' || !scope.sectionId) throw new Error('Visual editor requires section scope');
  if (String(change.sectionId || '') !== scope.sectionId) throw new Error('Visual editor section mismatch');
  const baseSection = readCommunityTrackerSection(scope.sectionId);
  if (!baseSection) throw new Error('Visual editor section not found');
  const section = resolveEffectiveCommunitySection(scope.sectionId);
  const communityVersion = Number(section && section.community && section.community.version) || 0;
  const requestedCommunityVersion = Number(change.source && change.source.communityVersion);
  if (Number.isInteger(requestedCommunityVersion) && requestedCommunityVersion !== communityVersion) {
    throw new Error('Участок уже изменился. Обновите актуальную версию и повторите правку');
  }
  const elements = Array.isArray(section.elements) ? section.elements : [];
  const starts = elements.map((item) => Number(item && item.start_m)).filter(Number.isFinite);
  const ends = elements.map((item) => Number(item && item.start_m) + Number(item && item.len_m)).filter(Number.isFinite);
  const coverage = section.runtime && Array.isArray(section.runtime.profile_coverage) && section.runtime.profile_coverage[0];
  const sectionStart = coverage && Number.isFinite(Number(coverage.start_m)) ? Number(coverage.start_m)
    : (Number.isFinite(Number(section.km_start)) ? Number(section.km_start) * 1000 : Math.min(...starts));
  const sectionEnd = coverage && Number.isFinite(Number(coverage.end_m)) ? Number(coverage.end_m)
    : (Number.isFinite(Number(section.km_end)) ? Number(section.km_end) * 1000 : Math.max(...ends));
  ...
  const normalized = {
    ...
    source: {
      schemaVersion: sanitizeProfileText(change.source && change.source.schemaVersion, 40),
      profileStatus: sanitizeProfileText(change.source && change.source.profileStatus, 120),
      communityVersion,
    },
  };
  if (kind !== 'object') {
    if (!Number.isInteger(startM) || !Number.isInteger(endM) || startM % 100 !== 0 || endM % 100 !== 0) {
      throw new Error('Visual editor boundaries must use 100 meter steps');
    }
    if (startM < sectionStart || endM > sectionEnd || endM <= startM) throw new Error('Visual editor boundaries outside section');
    if (endM - startM > 20000) throw new Error('Visual editor selection exceeds 20 km');
    normalized.startM = startM;
    normalized.endM = endM;
  }
  let riskLevel = '';
  if (kind === 'profile') {
    const fromGrade = Number(change.fromGrade);
    const toGrade = Number(change.toGrade);
    if (!Number.isFinite(fromGrade) || !Number.isFinite(toGrade) || Math.abs(fromGrade) > 45 || Math.abs(toGrade) > 45) {
      throw new Error('Invalid visual editor grade');
    }
    let weightedGrade = 0;
    let coveredLength = 0;
    elements.forEach((item) => {
      const itemStart = Number(item && item.start_m);
      const itemEnd = itemStart + Number(item && item.len_m);
      const grade = Number(item && item.grad_permille);
      if (!Number.isFinite(itemStart) || !Number.isFinite(itemEnd) || !Number.isFinite(grade)) return;
      const overlap = Math.max(0, Math.min(endM, itemEnd) - Math.max(startM, itemStart));
      weightedGrade += grade * overlap;
      coveredLength += overlap;
    });
    if (!coveredLength) throw new Error('Visual editor selection has no profile data');
    const authoritativeFromGrade = Math.round(weightedGrade / coveredLength * 10) / 10;
    if (Math.abs(authoritativeFromGrade - toGrade) < 0.05) throw new Error('Visual editor grade is unchanged');
    normalized.fromGrade = authoritativeFromGrade;
    normalized.toGrade = Math.round(toGrade * 10) / 10;
  } else if (kind === 'speed') {
    const fromSpeed = Number(change.fromSpeed);
    const toSpeed = Number(change.toSpeed);
    const action = sanitizeProfileText(change.action, 20).toLowerCase();
    if (!Number.isFinite(fromSpeed) || fromSpeed <= 0 || fromSpeed > 160) throw new Error('Invalid current speed');
    if (action !== 'set' && action !== 'remove') throw new Error('Invalid speed action');
    if (action === 'set' && (!Number.isFinite(toSpeed) || toSpeed <= 0 || toSpeed > 160 || toSpeed === fromSpeed)) {
      throw new Error('Invalid new speed');
    }
    normalized.fromSpeed = Math.round(fromSpeed);
    normalized.toSpeed = action === 'remove' ? Math.round(fromSpeed) : Math.round(toSpeed);
    normalized.action = action;
  } else if (kind === 'object') {
    const action = sanitizeProfileText(change.action, 20).toLowerCase();
    if (!['add', 'update', 'remove'].includes(action)) throw new Error('Invalid map object action');
    const existingObjects = listCommunitySectionObjects(section);
    let sourceObject = null;
    if (action !== 'add') {
      const source = change.sourceObject && typeof change.sourceObject === 'object' ? change.sourceObject : {};
      const collection = sanitizeProfileText(source.collection, 40);
      const requestedObjectKey = sanitizeProfileText(source.objectKey == null ? source.communityKey : source.objectKey, 200);
      const index = Number(source.index);
      if (!requestedObjectKey && (!Number.isInteger(index) || index < 0)) throw new Error('Invalid source map object');
      sourceObject = existingObjects.find((item) => requestedObjectKey
        ? item.objectKey === requestedObjectKey
        : item.collection === collection && item.index === index) || null;
      if (!sourceObject) throw new Error('Source map object not found');
      normalized.sourceObject = { ...sourceObject };
    }

    const requestedCoordinate = action === 'remove' && sourceObject ? sourceObject.coordinateM : Number(change.coordinateM);
    if (!Number.isInteger(requestedCoordinate) || (action !== 'remove' && requestedCoordinate % 10 !== 0)) {
      throw new Error('Map object coordinate must use 10 meter steps');
    }
    if (requestedCoordinate < sectionStart || requestedCoordinate > sectionEnd) throw new Error('Map object outside section');
    normalized.coordinateM = requestedCoordinate;
    normalized.action = action;

    if (action !== 'remove') {
      const rawObject = change.object && typeof change.object === 'object' ? change.object : {};
      const objectKind = sanitizeProfileText(rawObject.kind, 40).toLowerCase();
      const definition = COMMUNITY_OBJECT_TYPES.get(objectKind);
      if (!definition) throw new Error('Invalid map object type');
      const name = sanitizeProfileText(rawObject.name, 80) || definition.defaultName;
      const direction = sanitizeProfileText(rawObject.direction, 12).toLowerCase() || 'both';
      if (!['both', 'odd', 'even'].includes(direction)) throw new Error('Invalid map object direction');
      normalized.object = {
        kind: objectKind,
      ...
      const duplicate = existingObjects.some((item) =>
        (!sourceObject || item.collection !== sourceObject.collection || item.index !== sourceObject.index) &&
        item.kind === objectKind && item.name.toLowerCase() === name.toLowerCase() && Math.abs(item.coordinateM - requestedCoordinate) <= 20
      );
      if (duplicate) throw new Error('Map object already exists at this place');
      if (action === 'update' && sourceObject && sourceObject.kind === objectKind && sourceObject.name === name &&
          sourceObject.direction === direction && sourceObject.coordinateM === requestedCoordinate) {
        throw new Error('Map object is unchanged');
      }
    }

    if (action === 'add' && normalized.object && COMMUNITY_RESTRICTIVE_OBJECT_TYPES.has(normalized.object.kind)) {
      riskLevel = 'safety_restriction';
    } else if (sourceObject && COMMUNITY_SAFETY_OBJECT_TYPES.has(sourceObject.kind)) {
      riskLevel = 'safety_increase';
    }
  } else if (kind === 'geometry') {
    const action = sanitizeProfileText(change.action, 32).toLowerCase();
    if (action !== 'replace_fragment') throw new Error('Invalid geometry action');
    const pathId = sanitizeProfileText(change.pathId, 160);
    const paths = section.geometry && Array.isArray(section.geometry.paths) ? section.geometry.paths : [];
    const pathItem = paths.find((item) => String(item && item.path_id || '') === pathId);
    if (!pathItem || !Array.isArray(pathItem.points) || pathItem.points.length < 2) throw new Error('Geometry path not found');
    const pathPoints = pathItem.points.map((point) => ({
      chainageM: Number(point && (point.chainage_m == null ? point.ordinate : point.chainage_m)),
      ordinate: Number(point && point.ordinate),
      lat: Number(point && point.lat),
      lon: Number(point && point.lon),
    })).filter((point) => Number.isFinite(point.chainageM) && Number.isFinite(point.lat) && Number.isFinite(point.lon))
      .sort((a, b) => a.chainageM - b.chainageM);
    if (pathPoints.length < 2 || startM < pathPoints[0].chainageM || endM > pathPoints[pathPoints.length - 1].chainageM) {
      throw new Error('Geometry fragment outside selected path');
    }
    const interpolatePathPoint = (coordinate) => {
      let rightIndex = pathPoints.findIndex((point) => point.chainageM >= coordinate);
      if (rightIndex < 0) rightIndex = pathPoints.length - 1;
      const right = pathPoints[rightIndex];
      const left = pathPoints[Math.max(0, rightIndex - 1)];
      if (!left || !right) return null;
      const span = right.chainageM - left.chainageM;
      const ratio = span > 0 ? Math.max(0, Math.min(1, (coordinate - left.chainageM) / span)) : 0;
      return {
        chainageM: coordinate,
        ...
        ordinate: Number.isFinite(left.ordinate) && Number.isFinite(right.ordinate)
          ? Math.round(left.ordinate + (right.ordinate - left.ordinate) * ratio) : coordinate,
      };
    };
    const rawPoints = Array.isArray(change.points) ? change.points : [];
    if (rawPoints.length < 1 || rawPoints.length > 78) throw new Error('Geometry fragment must contain 1 to 78 control points');
    const unique = new Map();
    rawPoints.forEach((point) => {
      const chainageM = Math.round(Number(point && (point.chainageM == null ? point.chainage_m : point.chainageM)));
      const lat = Number(point && point.lat);
      const lon = Number(point && point.lon);
      if (!Number.isInteger(chainageM) || chainageM <= startM || chainageM >= endM || chainageM % 10 !== 0) {
        throw new Error('Geometry control points must use 10 meter steps inside fragment');
      }
      if (!Number.isFinite(lat) || !Number.isFinite(lon) || lat < -90 || lat > 90 || lon < -180 || lon > 180) {
        throw new Error('Invalid geometry coordinates');
      }
      const current = interpolatePathPoint(chainageM);
      if (!current || communityGeoDistanceMeters(current.lat, current.lon, lat, lon) > 3000) {
        throw new Error('Geometry control point is too far from published path');
      }
      const accuracyM = Number(point && point.accuracyM);
      unique.set(chainageM, {
        chainageM,
        lat: Math.round(lat * 1e7) / 1e7,
        lon: Math.round(lon * 1e7) / 1e7,
        ordinate: current.ordinate,
        accuracyM: Number.isFinite(accuracyM) ? Math.max(0, Math.min(200, Math.round(accuracyM))) : 0,
      });
    });
    const points = Array.from(unique.values()).sort((a, b) => a.chainageM - b.chainageM);
    if (!points.length) throw new Error('Geometry fragment has no unique control points');
    const startPoint = interpolatePathPoint(startM);
    const endPoint = interpolatePathPoint(endM);
    const changed = points.some((point) => {
      const current = interpolatePathPoint(point.chainageM);
      return current && communityGeoDistanceMeters(current.lat, current.lon, point.lat, point.lon) >= 3;
    });
    if (!changed) throw new Error('Geometry fragment is unchanged');
    normalized.action = action;
    normalized.pathId = pathId;
    normalized.points = [startPoint].concat(points, [endPoint]);
    normalized.sourcePath = {
      pathId,
      pointCount: pathPoints.length,
      geometryStatus: sanitizeProfileText(section.geometry && section.geometry.status, 120),
    };
    riskLevel = 'safety_increase';
  } else {
    throw new Error('Visual editor supports profile, speed, geometry or map object changes');
  }
  return {
    change: normalized,
    baseVersion: communitySectionResolver.sectionBaseVersion(baseSection),
    riskLevel,
  };
}

function resolveCommunityProfileContext(sid) {
  const profile = readProfile(sid);
  const catalog = readCommunityCatalog();
  const depot = findCommunityDepot(catalog, profile.depotId);
  const railwayId = depot && depot.railway_id ? String(depot.railway_id) : String(profile.railwayId || '');
  const railway = findCommunityRailway(catalog, railwayId);
  const pack = depot ? readCommunityDepotPack(depot) : null;
  return { profile, catalog, railway, depot, pack };
}

function syncCommunityMembershipFromProfile(sid, context) {
  const userId = normalizeSid(sid);
  const resolved = context || resolveCommunityProfileContext(userId);
  if (!resolved.depot || !resolved.railway) return null;
  const storage = getStorage();
  const memberships = storage.listCommunityMembershipsForSid(userId);
  const existing = memberships.find((item) => item.scopeLevel === 'depot' && item.scopeKey === resolved.depot.id);
  if (existing && existing.primary) return existing;
  storage.clearCommunityPrimaryMemberships(userId);
  const membership = storage.upsertCommunityMembership({
    sid: userId,
    scopeLevel: 'depot',
    scopeKey: String(resolved.depot.id),
    railwayId: String(resolved.railway.id),
    depotId: String(resolved.depot.id),
    status: existing ? existing.status : 'selected',
    role: existing ? existing.role : 'member',
    roleSource: existing ? existing.roleSource : 'self',
    termEndsAt: existing ? existing.termEndsAt : '',
    primary: true,
  });
  if (!existing) {
    storage.recordCommunityAudit({
      ...
      entityId: `${userId}:depot:${resolved.depot.id}`,
      scopeKey: resolved.depot.id,
      payload: { railwayId: resolved.railway.id, depotId: resolved.depot.id },
    });
  }
  return membership;
}

function communityScopeMatchesContext(scope, context) {
  if (!scope || !context) return false;
  if (scope.level === 'network') return true;
  if (scope.railwayId && String(scope.railwayId) !== String(context.railway && context.railway.id || '')) return false;
  if (scope.level === 'railway') return !!context.railway;
  if (scope.depotId && String(scope.depotId) !== String(context.depot && context.depot.id || '')) return false;
  if (scope.level === 'depot') return !!context.depot;
  if (scope.level === 'service_arm') {
    return !!(context.pack && Array.isArray(context.pack.service_arms) && context.pack.service_arms.some((arm) =>
      arm && String(arm.id || '') === String(scope.serviceArmId || '')
    ));
  }
  if (scope.level === 'section') {
    return !!(context.pack && Array.isArray(context.pack.section_ids) && context.pack.section_ids.some((id) =>
      String(id) === String(scope.sectionId || '')
    ));
  }
  return false;
}

function communityMembershipCoversScope(membership, scope) {
  if (!membership || !scope || membership.status !== 'verified') return false;
  if (membership.scopeLevel === 'railway') return membership.railwayId === scope.railwayId;
  if (membership.scopeLevel === 'depot') return membership.depotId === scope.depotId;
  if (membership.scopeLevel === 'service_arm') {
    return membership.depotId === scope.depotId && membership.serviceArmId === scope.serviceArmId;
  }
  return false;
}

function getCommunityAuthority(sid, scope, options) {
  const requireReviewer = !!(options && options.requireReviewer);
  const memberships = getStorage().listCommunityMembershipsForSid(normalizeSid(sid));
  const applicable = memberships.filter((membership) => communityMembershipCoversScope(membership, scope));
  const ranked = applicable.sort((a, b) => {
    const roleRank = { member: 1, reviewer: 2, curator: 3 };
    return (roleRank[b.role] || 0) - (roleRank[a.role] || 0);
  });
  const membership = ranked[0] || null;
  if (!membership) return null;
  if (requireReviewer && membership.role !== 'reviewer' && membership.role !== 'curator') return null;
  return membership;
}

function sanitizeCommunityScope(rawScope, context, isAdmin) {
  const source = rawScope && typeof rawScope === 'object' && !Array.isArray(rawScope) ? rawScope : {};
  const requestedLevel = sanitizeProfileText(source.level, 24);
  const level = COMMUNITY_SCOPE_LEVELS.has(requestedLevel) ? requestedLevel : 'depot';
  const railwayId = sanitizeProfileText(source.railwayId == null ? source.railway_id : source.railwayId, 80) ||
    String(context.railway && context.railway.id || '');
  const depotId = sanitizeProfileText(source.depotId == null ? source.depot_id : source.depotId, 160) ||
    String(context.depot && context.depot.id || '');
  const serviceArmId = sanitizeProfileText(source.serviceArmId == null ? source.service_arm_id : source.serviceArmId, 160);
  const sectionId = sanitizeProfileText(source.sectionId == null ? source.section_id : source.sectionId, 160);
  const scope = { level, railwayId, depotId, serviceArmId, sectionId };
  if (level === 'network' && !isAdmin) throw new Error('Invalid scope: network proposals require admin');
  if ((level === 'railway' || level === 'depot' || level === 'service_arm' || level === 'section') && !findCommunityRailway(context.catalog, railwayId)) {
    throw new Error('Invalid scope railway');
  }
  const depot = level === 'depot' || level === 'service_arm' || level === 'section'
    ? findCommunityDepot(context.catalog, depotId)
    : null;
  if ((level === 'depot' || level === 'service_arm' || level === 'section') && !depot) throw new Error('Invalid scope depot');
  if (!isAdmin && level !== 'network') {
    if (railwayId !== String(context.railway && context.railway.id || '')) throw new Error('Forbidden scope railway');
    if ((level === 'depot' || level === 'service_arm' || level === 'section') && depotId !== String(context.depot && context.depot.id || '')) {
      throw new Error('Forbidden scope depot');
    }
  }
  const pack = depot ? readCommunityDepotPack(depot) : context.pack;
  if (level === 'service_arm') {
    if (!serviceArmId || !pack || !Array.isArray(pack.service_arms) || !pack.service_arms.some((arm) => String(arm && arm.id || '') === serviceArmId)) {
      throw new Error('Invalid scope service arm');
    }
  }
  if (level === 'section') {
    if (!sectionId || !pack || !Array.isArray(pack.section_ids) || !pack.section_ids.some((id) => String(id) === sectionId)) {
      throw new Error('Invalid scope section');
    }
  }
  scope.key = communityScopeKey(scope);
  if (!scope.key) throw new Error('Invalid scope');
  return scope;
}

function deriveCommunityRiskLevel(kind, requestedRisk, change) {
  const explicit = COMMUNITY_RISK_LEVELS.has(requestedRisk) ? requestedRisk : 'normal';
  if (kind !== 'speed') return explicit === 'safety_increase' ? 'normal' : explicit;
  const fromSpeed = Number(change && (change.fromSpeed == null ? change.from_speed : change.fromSpeed));
  const toSpeed = Number(change && (change.toSpeed == null ? change.to_speed : change.toSpeed));
  const action = sanitizeProfileText(change && change.action, 40).toLowerCase();
  if (action === 'remove' || action === 'cancel' || action === 'withdraw' ||
      (Number.isFinite(fromSpeed) && Number.isFinite(toSpeed) && toSpeed > fromSpeed)) return 'safety_increase';
  if (Number.isFinite(fromSpeed) && Number.isFinite(toSpeed) && toSpeed < fromSpeed) return 'safety_restriction';
  return explicit;
}

function hasCommunitySafetyEvidence(evidence) {
  if (!evidence || typeof evidence !== 'object') return false;
  return ['orderNumber', 'sourceReference', 'documentRequestId', 'order_number', 'source_reference', 'document_request_id']
    .some((key) => sanitizeProfileText(evidence[key], 240));
}

function sanitizeCommunityRollbackChange(kind, change, scope) {
  if (!change || change.editor !== 'rollback-v1') return null;
  if (kind !== 'section') throw new Error('Rollback proposal must use section kind');
  if (scope.level !== 'section' || !scope.sectionId) throw new Error('Rollback requires section scope');
  if (String(change.sectionId || '') !== scope.sectionId) throw new Error('Rollback section mismatch');
  if (sanitizeProfileText(change.action, 24).toLowerCase() !== 'rollback') throw new Error('Invalid rollback action');
  const targetReleaseId = sanitizeProfileText(change.targetReleaseId, 80);
  if (!/^release:[a-f0-9]{24}$/.test(targetReleaseId)) throw new Error('Invalid rollback release');
  const releases = listPublishedCommunitySectionReleases(scope.sectionId);
  const target = releases.find((release) => release.id === targetReleaseId);
  if (!target || communitySectionResolver.rollbackTargetId(target)) throw new Error('Rollback target not found');
  if (releases.some((release) => communitySectionResolver.rollbackTargetId(release) === targetReleaseId)) {
    throw new Error('Эта правка уже отменена');
  }
  const section = resolveEffectiveCommunitySection(scope.sectionId);
  const communityVersion = Number(section && section.community && section.community.version) || 0;
  const requestedCommunityVersion = Number(change.source && change.source.communityVersion);
  if (Number.isInteger(requestedCommunityVersion) && requestedCommunityVersion !== communityVersion) {
    throw new Error('Участок уже изменился. Обновите актуальную версию и повторите отмену');
  }
  const targetRisk = String(target.payload && target.payload.riskLevel || 'normal');
  const riskLevel = targetRisk === 'safety_restriction'
    ? 'safety_increase'
    : (targetRisk === 'safety_increase' ? 'safety_restriction' : 'normal');
  const baseSection = readCommunityTrackerSection(scope.sectionId);
  return {
    change: {
      editor: 'rollback-v1',
...
function sanitizeCommunityProposal(payload, sid, requestUser) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new Error('Expected JSON object payload');
  const kind = sanitizeProfileText(payload.kind, 32);
  if (!COMMUNITY_KINDS.has(kind)) throw new Error('Invalid proposal kind');
  const context = resolveCommunityProfileContext(sid);
  if (!context.depot || !context.railway) throw new Error('Missing canonical railway or depot in profile');
  syncCommunityMembershipFromProfile(sid, context);
  const scope = sanitizeCommunityScope(payload.scope, context, isAnalyticsAdmin(requestUser));
  const rawChange = sanitizeCommunityJsonObject(payload.change, COMMUNITY_MAX_CHANGE_BYTES, 'change');
  const normalizedChange = sanitizeCommunityRollbackChange(kind, rawChange, scope) || sanitizeCommunityVisualChange(kind, rawChange, scope);
  const change = normalizedChange.change;
  const evidence = sanitizeCommunityJsonObject(payload.evidence, COMMUNITY_MAX_EVIDENCE_BYTES, 'evidence');
  const riskLevel = normalizedChange.riskLevel || deriveCommunityRiskLevel(kind, sanitizeProfileText(payload.riskLevel || payload.risk_level, 32), change);
  if (riskLevel === 'safety_increase' && !hasCommunitySafetyEvidence(evidence)) {
    throw new Error('Missing documentary evidence for safety-sensitive change');
  }
  const title = sanitizeProfileText(payload.title, 160);
  if (!title) throw new Error('Missing proposal title');
  return {
    id: `proposal:${crypto.randomBytes(12).toString('hex')}`,
    authorSid: normalizeSid(sid),
    kind,
    riskLevel,
    scope,
    title,
    summary: sanitizeProfileText(payload.summary, 1000),
    baseVersion: normalizedChange.baseVersion || sanitizeProfileText(payload.baseVersion == null ? payload.base_version : payload.baseVersion, 160),
    change,
    evidence,
    status: 'reviewing',
  };
...
function communityReviewSummary(proposalId) {
  const reviews = getStorage().listCommunityReviews(proposalId);
  const result = { total: reviews.length, confirm: 0, needsFix: 0, reject: 0, abstain: 0, confirmWeight: 0, concernWeight: 0 };
  reviews.forEach((review) => {
    if (review.verdict === 'confirm') {
      result.confirm += 1;
      result.confirmWeight += review.weight;
    } else if (review.verdict === 'needs_fix') {
      result.needsFix += 1;
      result.concernWeight += review.weight;
...
function evaluateCommunityProposalStatus(proposal) {
  const summary = communityReviewSummary(proposal.id);
  const threshold = proposal.riskLevel === 'safety_restriction' ? 2 : proposal.riskLevel === 'safety_increase' ? 4 : 3;
  let status = proposal.status;
  if (summary.concernWeight >= 2 || (summary.reject && summary.confirm)) status = 'disputed';
  else if (summary.needsFix > 0 || summary.reject > 0) status = 'needs_info';
  else if (summary.confirmWeight >= threshold) status = 'accepted';
  else status = 'reviewing';
  if (status !== proposal.status) return getStorage().updateCommunityProposalStatus(proposal.id, status);
  return proposal;
}

function publishCommunityAcceptedProposal(proposal, actorSid) {
  if (!proposal || proposal.status !== 'accepted') return proposal;
  const storage = getStorage();
  const existing = storage.listCommunityReleases(1000).find((release) => release.proposalId === proposal.id);
  if (existing) return storage.updateCommunityProposalStatus(proposal.id, 'published');
  const scoped = storage.listCommunityReleases(1000)
    .filter((release) => release.scopeLevel === proposal.scope.level && release.scopeKey === proposal.scope.key);
  const version = scoped.reduce((max, release) => Math.max(max, Number(release.version) || 0), 0) + 1;
  const release = storage.createCommunityRelease({
    id: `release:${crypto.randomBytes(12).toString('hex')}`,
  ...
  const published = storage.updateCommunityProposalStatus(proposal.id, 'published');
  storage.recordCommunityAudit({
    actorSid,
    action: 'proposal.auto_published',
    entityType: 'release',
    entityId: release.id,
    scopeKey: proposal.scope.key,
    payload: { proposalId: proposal.id, version, payloadSha256: release.payloadSha256 },
  });
  return published;
}

function communityAuthorLabel(sid) {
  const profile = readProfile(sid);
  return [profile.firstName, profile.lastName].filter(Boolean).join(' ').trim() || 'Коллега';
}

function publicCommunityProposal(proposal, viewerSid) {
  const summary = communityReviewSummary(proposal.id);
  return {
    id: proposal.id,
    kind: proposal.kind,
    ...
    author: communityAuthorLabel(proposal.authorSid),
    authorKey: crypto.createHash('sha256').update(proposal.authorSid).digest('hex').slice(0, 12),
    own: proposal.authorSid === normalizeSid(viewerSid),
    reviews: summary,
...
function buildCommunityDashboard(sid) {
  const userId = normalizeSid(sid);
  const context = resolveCommunityProfileContext(userId);
  const primaryMembership = syncCommunityMembershipFromProfile(userId, context);
  const memberships = getStorage().listCommunityMembershipsForSid(userId);
  const proposals = getStorage().listCommunityProposals(1000);
  const applicable = proposals.filter((proposal) => proposal.authorSid === userId || communityScopeMatchesContext(proposal.scope, context));
  const reviewable = applicable.filter((proposal) =>
    proposal.authorSid !== userId &&
    ['reviewing', 'needs_info', 'disputed'].includes(proposal.status) &&
    !!getCommunityAuthority(userId, proposal.scope, { requireReviewer: true })
  );
  const uploads = readDepotPackRequests().filter((item) => item && item.sid === userId).slice(-20).reverse().map(publicDepotPackRequest);
  const serviceArms = context.pack && Array.isArray(context.pack.service_arms)
    ? context.pack.service_arms.map((arm) => ({
      id: String(arm.id || ''),
      name: String(arm.name || ''),
      description: String(arm.description || ''),
      routes: Array.isArray(arm.route_options) ? arm.route_options.length : 0,
    }))
    : [];
  const elections = getStorage().listCommunityElections(200).filter((election) =>
    election.status !== 'cancelled' && communityScopeMatchesContext({
      level: election.scopeLevel,
      railwayId: election.railwayId,
      depotId: election.depotId,
      serviceArmId: election.serviceArmId,
    }, context)
  ).map(publicCommunityElection);
  const releases = getStorage().listCommunityReleases(500).filter((release) =>
    communityScopeMatchesContext({
      level: release.scopeLevel,
      key: release.scopeKey,
      railwayId: release.payload && release.payload.scope && release.payload.scope.railwayId || '',
      depotId: release.payload && release.payload.scope && release.payload.scope.depotId || '',
      serviceArmId: release.payload && release.payload.scope && release.payload.scope.serviceArmId || '',
      sectionId: release.payload && release.payload.scope && release.payload.scope.sectionId || '',
    }, context)
  ).slice(0, 30);
  return {
    generatedAt: new Date().toISOString(),
    context: {
      configured: !!(context.railway && context.depot),
      railway: context.railway ? { id: context.railway.id, name: context.railway.name } : null,
      depot: context.depot ? { id: context.depot.id, code: context.depot.code, name: context.depot.name, status: context.depot.status } : null,
      pack: context.pack ? {
        id: context.pack.id,
        name: context.pack.name,
        status: context.pack.status,
        updatedAt: context.pack.updated_at || '',
        readiness: context.pack.readiness || {},
        sectionIds: Array.isArray(context.pack.section_ids) ? context.pack.section_ids.map(String) : [],
      } : null,
      serviceArms,
    },
    ...
    memberships,
    summary: {
      serviceArms: serviceArms.length,
      myProposals: applicable.filter((proposal) => proposal.authorSid === userId).length,
      reviewable: reviewable.length,
      openElections: elections.filter((election) => election.status === 'nominations' || election.status === 'voting').length,
      uploads: uploads.length,
    },
    myProposals: applicable.filter((proposal) => proposal.authorSid === userId).slice(0, 20).map((proposal) => publicCommunityProposal(proposal, userId)),
    reviewQueue: reviewable.slice(0, 20).map((proposal) => publicCommunityProposal(proposal, userId)),
    elections,
    releases,
    uploads,
  };
...
function communityCandidateKey(electionId, sid) {
  return crypto.createHash('sha256').update(`${electionId}:${normalizeSid(sid)}`).digest('hex').slice(0, 24);
}

function publicCommunityElection(election) {
  const turnout = election.status === 'closed' ? getStorage().countCommunityElectionVoters(election.id) : null;
  return {
    id: election.id,
    scopeLevel: election.scopeLevel,
    scopeKey: election.scopeKey,
    railwayId: election.railwayId,
    depotId: election.depotId,
    serviceArmId: election.serviceArmId,
    kind: election.kind,
    ...
    turnout,
    quorumMet: turnout == null ? null : turnout >= election.quorum,
    candidates: (election.candidates || []).map((candidate) => ({
      key: communityCandidateKey(election.id, candidate.candidateSid),
      name: communityAuthorLabel(candidate.candidateSid),
      statement: candidate.statement,
      status: candidate.status,
      votes: election.status === 'closed' ? candidate.votes : null,
    })),
  };
}

function communityElectionScope(election) {
  return {
    level: election.scopeLevel,
    key: election.scopeKey,
    railwayId: election.railwayId,
    depotId: election.depotId,
    serviceArmId: election.serviceArmId,
    sectionId: '',
...
function sanitizeCommunityElection(payload, sid) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new Error('Expected JSON object payload');
  const context = resolveCommunityProfileContext(sid);
  if (!context.depot || !context.railway) throw new Error('Missing canonical railway or depot in profile');
  syncCommunityMembershipFromProfile(sid, context);
  const scope = sanitizeCommunityScope(payload.scope, context, false);
  if (scope.level !== 'depot' && scope.level !== 'service_arm') throw new Error('Election scope must be depot or service arm');
  if (!getCommunityAuthority(sid, scope)) throw new Error('Verified membership required');
  const active = getStorage().listCommunityElections(500).find((item) =>
    item.scopeKey === scope.key && item.kind === 'curator' && ['nominations', 'voting'].includes(item.status)
  );
  if (active) throw new Error('Active election already exists for this scope');
  const now = Date.now();
  const startsAt = new Date(now).toISOString();
  const requestedEndsAt = Date.parse(String(payload.endsAt || payload.ends_at || ''));
  const endsAt = new Date(Number.isFinite(requestedEndsAt) && requestedEndsAt > now + 24 * 60 * 60 * 1000
    ? Math.min(requestedEndsAt, now + 30 * 24 * 60 * 60 * 1000)
    : now + 7 * 24 * 60 * 60 * 1000).toISOString();
  const verifiedInScope = getStorage().listCommunityMemberships(2000)
    .filter((membership) => communityMembershipCoversScope(membership, scope)).length;
  return {
    id: `election:${crypto.randomBytes(12).toString('hex')}`,
    ...
    kind: 'curator',
    status: 'nominations',
    seats: Math.max(2, Math.min(5, Number(payload.seats) || 3)),
    quorum: Math.max(2, Math.min(10000, Math.max(5, Math.ceil(verifiedInScope * 0.3)))),
    startsAt,
    endsAt,
    termEndsAt: new Date(Date.parse(endsAt) + 180 * 24 * 60 * 60 * 1000).toISOString(),
    createdBy: normalizeSid(sid),
  };
}

function sanitizeCommunityAdminMembership(payload) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new Error('Expected JSON object payload');
  const sid = normalizeSid(payload.sid);
  const scopeLevel = sanitizeProfileText(payload.scopeLevel || payload.scope_level, 24);
  const status = sanitizeProfileText(payload.status, 24);
  const role = sanitizeProfileText(payload.role, 24);
  if (!sid) throw new Error('Missing sid');
  if (!COMMUNITY_MEMBERSHIP_LEVELS.has(scopeLevel)) throw new Error('Invalid membership scope');
  if (!COMMUNITY_MEMBERSHIP_STATUSES.has(status)) throw new Error('Invalid membership status');
  if (!COMMUNITY_MEMBERSHIP_ROLES.has(role)) throw new Error('Invalid membership role');
  const catalog = readCommunityCatalog();
  const railwayId = sanitizeProfileText(payload.railwayId || payload.railway_id, 80);
  const depotId = sanitizeProfileText(payload.depotId || payload.depot_id, 160);
  const serviceArmId = sanitizeProfileText(payload.serviceArmId || payload.service_arm_id, 160);
  const railway = findCommunityRailway(catalog, railwayId);
  const depot = depotId ? findCommunityDepot(catalog, depotId) : null;
  if (!railway) throw new Error('Invalid membership railway');
  if ((scopeLevel === 'depot' || scopeLevel === 'service_arm') && (!depot || String(depot.railway_id || '') !== railwayId)) {
    throw new Error('Invalid membership depot');
  }
  if (scopeLevel === 'service_arm') {
    const pack = readCommunityDepotPack(depot);
    if (!serviceArmId || !pack || !Array.isArray(pack.service_arms) ||
        !pack.service_arms.some((arm) => String(arm && arm.id || '') === serviceArmId)) {
      throw new Error('Invalid membership service arm');
    }
  }
  const scopeKey = scopeLevel === 'railway' ? `railway:${railwayId}`
    : scopeLevel === 'depot' ? depotId : `service_arm:${depotId}:${serviceArmId}`;
  return {
    sid,
    scopeLevel,
    scopeKey,
    railwayId,
    depotId: scopeLevel === 'railway' ? '' : depotId,
    serviceArmId: scopeLevel === 'service_arm' ? serviceArmId : '',
    status,
    role,
    roleSource: 'admin',
    termEndsAt: sanitizeProfileText(payload.termEndsAt || payload.term_ends_at, 40),
    primary: payload.primary === true,
  };
}

function finalizeCommunityElection(election, actorSid) {
  const storage = getStorage();
  const turnout = storage.countCommunityElectionVoters(election.id);
  const candidates = storage.listCommunityCandidates(election.id)
    .filter((candidate) => candidate.status !== 'withdrawn');
  const winners = turnout >= election.quorum ? candidates.slice(0, election.seats) : [];
  const winnerSids = new Set(winners.map((candidate) => candidate.candidateSid));
  candidates.forEach((candidate) => {
    storage.updateCommunityCandidateStatus(
      election.id,
      candidate.candidateSid,
      winnerSids.has(candidate.candidateSid) ? 'elected' : 'not_elected'
    );
  });
  winners.forEach((candidate) => {
    storage.upsertCommunityMembership({
      sid: candidate.candidateSid,
      scopeLevel: election.scopeLevel,
      scopeKey: election.scopeKey,
      railwayId: election.railwayId,
      depotId: election.depotId,
  ...
  const closed = storage.updateCommunityElectionStatus(election.id, 'closed');
  storage.recordCommunityAudit({
    actorSid,
    ...
    scopeKey: election.scopeKey,
    payload: { turnout, quorum: election.quorum, winnerCount: winners.length },
  });
  return closed;
}

function buildCommunityAdminOverview() {
  const storageOverview = getStorage().buildCommunityAdminOverview();
  const catalog = readCommunityCatalog();
  const depotsWithPacks = catalog.depots.filter((depot) => depot && depot.pack_file).length;
  return {
    ...storageOverview,
    catalog: {
      railways: catalog.railways.length,
      depots: catalog.depots.length,
      depotsWithPacks,
      depotsWithoutPacks: Math.max(0, catalog.depots.length - depotsWithPacks),
    },
    uploadQueue: {
      total: readDepotPackRequests().length,
      open: readDepotPackRequests().filter((item) => item && ['new', 'reviewing', 'needs_info', 'accepted'].includes(item.status)).length,
    },
  };
}
...
const APP_RELEASE_VERSION = 'v413';
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/community-admin') {
    ...
    const dashboardPath = path.join(ROOT, 'admin', 'community.html');
    if (!fs.existsSync(dashboardPath)) {
      sendText(res, 404, 'Панель сообщества не найдена.');
      return;
    }
    sendText(res, 200, fs.readFileSync(dashboardPath, 'utf8'), 'text/html; charset=utf-8');
    return;
  ...
  if (pathname === '/api/community/dashboard') {
    if (!sid) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    try {
      sendJson(res, 200, buildCommunityDashboard(sid));
    } catch (err) {
      sendJson(res, 500, { error: err && err.message ? err.message : 'Community dashboard unavailable' });
    }
    return;
  }

  const communityEffectiveSectionMatch = pathname.match(/^\/api\/community\/sections\/([a-z0-9][a-z0-9-]{1,159})\/effective$/);
  if (communityEffectiveSectionMatch) {
    if (!sid) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    const sectionId = communityEffectiveSectionMatch[1];
    const context = resolveCommunityProfileContext(sid);
    if (!communityScopeMatchesContext({
      level: 'section',
      railwayId: String(context.railway && context.railway.id || ''),
    ...
    try {
      const section = resolveEffectiveCommunitySection(sectionId);
      if (!section) {
  ...
  if (pathname === '/api/community/proposals') {
    if (!sid) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    if (req.method === 'GET') {
      const context = resolveCommunityProfileContext(sid);
      const proposals = getStorage().listCommunityProposals(500)
        .filter((proposal) => proposal.authorSid === sid || communityScopeMatchesContext(proposal.scope, context))
        .map((proposal) => publicCommunityProposal(proposal, sid));
      sendJson(res, 200, { proposals, total: proposals.length });
      return;
    }
    if (req.method === 'POST') {
      if (!allowWindowedCount(communityProposalAttempts, sid, 1, COMMUNITY_PROPOSAL_RATE_MAX, COMMUNITY_PROPOSAL_RATE_WINDOW_MS)) {
        sendJson(res, 429, { error: 'Слишком много предложений. Попробуйте позже.' });
        return;
      }
      try {
        const body = await readBody(req);
        const proposal = getStorage().createCommunityProposal(
          sanitizeCommunityProposal(body ? JSON.parse(body) : {}, sid, requestUser)
        );
        getStorage().recordCommunityAudit({
          actorSid: sid,
          action: 'proposal.created',
          entityType: 'proposal',
          entityId: proposal.id,
          scopeKey: proposal.scope.key,
          payload: { kind: proposal.kind, riskLevel: proposal.riskLevel },
        });
        sendJson(res, 201, { ok: true, proposal: publicCommunityProposal(proposal, sid) });
      } catch (err) {
        const message = err && err.message ? err.message : 'Invalid proposal';
  ...
  const communityReviewMatch = pathname.match(/^\/api\/community\/proposals\/(proposal:[a-f0-9]{24})\/reviews$/);
  if (communityReviewMatch) {
    ...
    if (!allowWindowedCount(communityReviewAttempts, sid, 1, COMMUNITY_REVIEW_RATE_MAX, COMMUNITY_REVIEW_RATE_WINDOW_MS)) {
      sendJson(res, 429, { error: 'Слишком много проверок. Попробуйте позже.' });
      return;
    }
    const proposal = getStorage().readCommunityProposal(communityReviewMatch[1]);
    if (!proposal) {
      sendJson(res, 404, { error: 'Proposal not found' });
      return;
    }
    const context = resolveCommunityProfileContext(sid);
    if (!communityScopeMatchesContext(proposal.scope, context)) {
      sendJson(res, 403, { error: 'Proposal is outside your scope' });
      return;
    }
    if (proposal.authorSid === sid) {
      sendJson(res, 403, { error: 'Нельзя проверять собственное предложение' });
      return;
    }
    if (!['reviewing', 'needs_info', 'disputed', 'accepted'].includes(proposal.status)) {
      sendJson(res, 409, { error: 'Proposal is not reviewable' });
      return;
    }
    const authority = getCommunityAuthority(sid, proposal.scope, { requireReviewer: true });
    if (!authority) {
      sendJson(res, 403, { error: 'Нужен подтверждённый статус проверяющего или куратора' });
      return;
    }
    try {
      const body = await readBody(req);
      const payload = body ? JSON.parse(body) : {};
      const verdict = sanitizeProfileText(payload.verdict, 24);
      if (!COMMUNITY_REVIEW_VERDICTS.has(verdict)) throw new Error('Invalid review verdict');
      const review = getStorage().upsertCommunityReview({
        proposalId: proposal.id,
        reviewerSid: sid,
        verdict,
        notes: sanitizeProfileText(payload.notes, 1000),
        weight: authority.role === 'curator' ? 2 : 1,
        reviewerRole: authority.role,
      });
      const evaluated = evaluateCommunityProposalStatus(getStorage().readCommunityProposal(proposal.id));
      const finalized = publishCommunityAcceptedProposal(evaluated, sid);
      getStorage().recordCommunityAudit({
        actorSid: sid,
        ...
        entityId: proposal.id,
        scopeKey: proposal.scope.key,
        payload: { verdict, reviewerRole: authority.role, weight: review.weight, status: finalized.status },
      });
      sendJson(res, 200, { ok: true, review: { verdict: review.verdict, notes: review.notes, weight: review.weight }, proposal: publicCommunityProposal(finalized, sid) });
    } catch (err) {
      sendJson(res, 400, { error: err && err.message ? err.message : 'Invalid review' });
    }
    return;
  }

  if (pathname === '/api/community/releases') {
    if (!sid) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    const context = resolveCommunityProfileContext(sid);
    const releases = getStorage().listCommunityReleases(1000).filter((release) => {
      const scope = release.payload && release.payload.scope || {};
      return communityScopeMatchesContext({
        level: release.scopeLevel,
        key: release.scopeKey,
        railwayId: scope.railwayId || '',
        depotId: scope.depotId || '',
    ...
    sendJson(res, 200, { releases, total: releases.length });
    return;
  }

  if (pathname === '/api/community/elections') {
    if (!sid) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    if (req.method === 'GET') {
      const context = resolveCommunityProfileContext(sid);
      const elections = getStorage().listCommunityElections(200)
        .filter((election) => communityScopeMatchesContext(communityElectionScope(election), context))
        .map(publicCommunityElection);
      sendJson(res, 200, { elections, total: elections.length });
      return;
    }
    if (req.method === 'POST') {
      try {
        const body = await readBody(req);
        const election = getStorage().createCommunityElection(sanitizeCommunityElection(body ? JSON.parse(body) : {}, sid));
        getStorage().recordCommunityAudit({
          actorSid: sid,
          ...
          entityId: election.id,
          scopeKey: election.scopeKey,
          payload: { seats: election.seats, quorum: election.quorum },
        });
        sendJson(res, 201, { ok: true, election: publicCommunityElection(election) });
      } catch (err) {
        const message = err && err.message ? err.message : 'Invalid election';
        sendJson(res, /required|Forbidden/i.test(message) ? 403 : (/already exists/.test(message) ? 409 : 400), { error: message });
      }
      return;
    }
    sendJson(res, 405, { error: 'Method not allowed' });
  ...
  const communityCandidateMatch = pathname.match(/^\/api\/community\/elections\/(election:[a-f0-9]{24})\/candidates$/);
  if (communityCandidateMatch) {
    if (!sid) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    const election = getStorage().readCommunityElection(communityCandidateMatch[1]);
    if (!election) {
      sendJson(res, 404, { error: 'Election not found' });
      return;
    }
    if (election.status !== 'nominations') {
      sendJson(res, 409, { error: 'Nominations are closed' });
      return;
    }
    const authority = getCommunityAuthority(sid, communityElectionScope(election));
    if (!authority) {
      sendJson(res, 403, { error: 'Verified membership required' });
      return;
    }
    try {
      const body = await readBody(req);
      const payload = body ? JSON.parse(body) : {};
      const candidate = getStorage().upsertCommunityCandidate({
        electionId: election.id,
        candidateSid: sid,
        nominatedBy: sid,
        statement: sanitizeProfileText(payload.statement, 500),
        status: 'accepted',
      });
      getStorage().recordCommunityAudit({
        actorSid: sid,
        action: 'election.self_nominated',
        entityType: 'election',
        entityId: election.id,
        scopeKey: election.scopeKey,
        payload: {},
      });
      sendJson(res, 200, { ok: true, candidate: { key: communityCandidateKey(election.id, candidate.candidateSid), name: communityAuthorLabel(sid), statement: candidate.statement } });
    } catch (err) {
      sendJson(res, 400, { error: err && err.message ? err.message : 'Invalid nomination' });
    }
    return;
  }

  const communityBallotMatch = pathname.match(/^\/api\/community\/elections\/(election:[a-f0-9]{24})\/ballot$/);
  if (communityBallotMatch) {
    if (!sid) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    const election = getStorage().readCommunityElection(communityBallotMatch[1]);
    if (!election) {
      sendJson(res, 404, { error: 'Election not found' });
      return;
    }
    if (election.status !== 'voting') {
      sendJson(res, 409, { error: 'Voting is not active' });
      return;
    }
    if (!getCommunityAuthority(sid, communityElectionScope(election))) {
      sendJson(res, 403, { error: 'Verified membership required' });
      return;
    }
    try {
      const body = await readBody(req);
      const payload = body ? JSON.parse(body) : {};
      const keys = Array.from(new Set((Array.isArray(payload.candidateKeys) ? payload.candidateKeys : []).map(String)));
      if (!keys.length || keys.length > election.seats) throw new Error(`Choose from 1 to ${election.seats} candidates`);
      const eligibleCandidates = election.candidates.filter((candidate) => candidate.status === 'accepted' || candidate.status === 'nominated');
      const byKey = new Map(eligibleCandidates.map((candidate) => [communityCandidateKey(election.id, candidate.candidateSid), candidate.candidateSid]));
      const candidateSids = keys.map((key) => byKey.get(key));
      if (candidateSids.some((candidateSid) => !candidateSid)) throw new Error('Invalid candidate choice');
      const ballot = getStorage().replaceCommunityBallot(election.id, sid, candidateSids);
      getStorage().recordCommunityAudit({
        actorSid: sid,
        action: 'election.ballot_replaced',
        entityType: 'election',
        entityId: election.id,
        scopeKey: election.scopeKey,
        payload: { choices: ballot.choices },
      });
      sendJson(res, 200, { ok: true, choices: ballot.choices });
    } catch (err) {
      sendJson(res, 400, { error: err && err.message ? err.message : 'Invalid ballot' });
    }
    return;
  }

  if (pathname === '/api/admin/community') {
    if (!requestUser) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    sendJson(res, 200, buildCommunityAdminOverview());
    return;
  }

  if (pathname === '/api/admin/community/memberships') {
    if (!requestUser) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    try {
      const body = await readBody(req);
      const membership = getStorage().upsertCommunityMembership(sanitizeCommunityAdminMembership(body ? JSON.parse(body) : {}));
      getStorage().recordCommunityAudit({
        actorSid: sid || `admin:${requestUser.id}`,
        action: 'membership.admin_updated',
        entityType: 'membership',
        entityId: `${membership.sid}:${membership.scopeLevel}:${membership.scopeKey}`,
        scopeKey: membership.scopeKey,
        payload: { status: membership.status, role: membership.role },
      });
      sendJson(res, 200, { ok: true, membership });
    } catch (err) {
      sendJson(res, 400, { error: err && err.message ? err.message : 'Invalid membership' });
    }
    return;
  }

  const adminCommunityElectionMatch = pathname.match(/^\/api\/admin\/community\/elections\/(election:[a-f0-9]{24})$/);
  if (adminCommunityElectionMatch) {
    if (!requestUser) {
      sendJson(res, 401, { error: 'Unauthorized' });
      return;
    }
    ...
    const election = getStorage().readCommunityElection(adminCommunityElectionMatch[1]);
    if (!election) {
      sendJson(res, 404, { error: 'Election not found' });
      return;
    }
    try {
      ...
      const payload = body ? JSON.parse(body) : {};
      const status = sanitizeProfileText(payload.status, 24);
      if (!COMMUNITY_ELECTION_STATUSES.has(status) || status === 'draft') throw new Error('Invalid election status');
      if (status === 'voting' && !election.candidates.some((candidate) => candidate.status === 'accepted' || candidate.status === 'nominated')) {
        throw new Error('Election has no candidates');
      }
      const updated = status === 'closed'
        ? finalizeCommunityElection(election, sid || `admin:${requestUser.id}`)
        : getStorage().updateCommunityElectionStatus(election.id, status);
      if (status !== 'closed') {
        getStorage().recordCommunityAudit({
          actorSid: sid || `admin:${requestUser.id}`,
          ...
          entityId: election.id,
          scopeKey: election.scopeKey,
          payload: { from: election.status, to: status },
        });
      }
      sendJson(res, 200, { ok: true, election: publicCommunityElection(updated) });
    } catch (err) {
      sendJson(res, 400, { error: err && err.message ? err.message : 'Invalid election update' });
    }
    return;
  }
