...
## 2026-04-23 01:27:33 +0000

- Source: `manual`
- Task: Перевёл ручные schedule delete/edit на atomic local update
- Branch: `main`
- Methods: `добавил setScheduleDayOverrideLocal без немедленного remote save`, `delete/edit flows сначала локально меняют override и allShifts`, `потом пересобирают день и только затем отдельно сохраняют schedule store и shifts`, `bumped SW cache`
- Files: `scripts/app.js`, `scripts/shift-form.js`, `sw.js`

## 2026-04-23 01:54:32 +0000

- Source: `manual`
- Task: Убрал persisted materialized shifts из load/save и render side-effects
- Branch: `main`
- Methods: `clone/load paths теперь фильтруют schedule-generated rows`, `auth/render/month navigation больше не вызывают persistence ради отображения`, `schedule materialization начинает уходить из persisted truth`
- Files: `scripts/app.js`, `scripts/render.js`, `scripts/shift-form.js`, `scripts/auth.js`

## 2026-04-23 04:32:28 +0000

- Source: `manual`
- Task: Зафиксировал, что осталось доделать по рефактору графика
- Branch: `main`
- Methods: `записал текущий незавершенный scope: добить удаление legacy materialized-layer в app.js`, `проверить ручное удаление/замену дня графика`, `удалить график вместе со связанными ручными хвостами`, `довести календарные стрелки/бейдж хвоста и только после этого коммит/push`
- Files: `scripts/app.js`, `scripts/shift-form.js`, `scripts/render.js`, `styles/10-navigation-and-cards.css`, `sw.js`

## 2026-04-23 04:40:35 +0000

- Source: `manual`
- Task: Подчинил linked state графика при edit/replace period
- Branch: `main`
- Methods: `Добавил reconcileScheduleLinkedState: override и schedule-origin/manual хвосты теперь либо удаляются вместе с вырезанным куском периода`, `либо перепривязываются к новому right-tail periodId при replace`, `upsert existing period теперь тоже чистит state вне нового диапазона`
- Files: `scripts/app.js`

## 2026-04-23 05:32:57 +0000

- Source: `manual`
- Task: remove schedule-driven flows and switch UI to manual-only calendar
- Branch: `main`
- Methods: `disabled schedule planner/day overlays`, `removed derived schedule shifts from month calculations`, `stopped linking manual shifts to schedule metadata`, `switched home calendar clicks to manual add/open flows`, `skipped schedule reload on auth`
- Files: `index.html`, `scripts/app.js`, `scripts/auth.js`, `scripts/render.js`, `scripts/shift-form.js`

## 2026-04-23 05:41:31 +0000

- Source: `manual`
- Task: Убрал очевидный мёртвый schedule-код после manual-only перехода
- Branch: `main`
- Methods: `удалил неиспользуемые scheduleArchiveExpanded и hasScheduleStoreData из scripts/app.js после repo-wide usage review schedule-функционала`
- Files: `scripts/app.js`

## 2026-04-23 05:51:46 +0000

- Source: `manual`
- Task: Удалил мёртвый planner/day-overlay UI код после manual-only перехода
- Branch: `main`
- Methods: `вырезал JS/CSS ветки без DOM в index.html`, `оставил home calendar`, `journal и manual shift form`, `проверил остаточные ссылки grep и синтаксис node -c`
- Files: `scripts/render.js`, `scripts/shift-form.js`, `scripts/app.js`, `styles/10-navigation-and-cards.css`

## 2026-04-23 05:52:15 +0000

- Source: `manual`
- Task: Убрал фронтовый хвост schedule sync/store после manual-only
- Branch: `main`
- Methods: `удалил client-side schedule API/localStorage/pending-sync helpers`, `заменил saveScheduleStore на in-memory no-op`, `вырезал вызовы schedule flush retry из shift-form`, `убрал неиспользуемые schedule selection/conflict form helpers после repo-wide grep`
- Files: `scripts/app.js`, `scripts/auth.js`, `scripts/shift-form.js`

## 2026-04-23 05:58:46 +0000

- Source: `manual`
- Task: Очистил legacy schedule shifts из local JSON
- Branch: `main`
- Methods: `проверил data/local-shifts dry-run скриптом`, `добавил backup-safe tools/migrate_legacy_local_shifts.py`, `удалил только schedule_generated rows и сохранил .bak snapshot перед apply`, `проверил py_compile и отсутствие schedule_* в активных JSON`
- Files: `tools/migrate_legacy_local_shifts.py`, `data/local-shifts/906498745.json`, `data/local-shifts/906498745.json.bak.20260423T055822Z`

## 2026-04-23 05:59:32 +0000

- Source: `manual`
- Task: Removed backend schedule storage and replaced /api/schedule with legacy empty shim
- Branch: `main`
- Methods: `Deleted local-schedules directory wiring/validators/read-write helpers from server.js after confirming frontend no longer calls /api/schedule`, `kept authenticated GET/PUT shim returning empty schedule/no-op for stale cached clients`
- Files: `server.js`
