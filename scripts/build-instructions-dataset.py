...
def split_number_title(value: str) -> Tuple[str, str]:
    ...
    patterns = [
        r"^(Приложение\s+N\s*\d+[а-яА-ЯA-Za-z0-9-]*)\.\s*(.+)$",
        r"^([IVXLCDM]+)\.\s+(.+)$",
        r"^(\d+(?:\.\d+)*)\.\s+(.+)$",
        r"^(\d+(?:\.\d+)*)\)\s+(.+)$",
    ]
...
def extract_numbered_points(text: str) -> List[Tuple[str, str, str]]:
    if not text:
        return []
    paragraphs = [normalize_space(p) for p in re.split(r"\n{2,}", text) if normalize_space(p)]
...
def build_instruction(
    ...
    for item in branch:
        ...
        points = extract_numbered_points(content)
        for point_number, point_body, point_content in points:
            point_order = sibling_counters.get(node_id, 0) + 1
            sibling_counters[node_id] = point_order
            point_id = (
                f"{node_id}-p-"
                f"{hashlib.sha1((point_number + point_body).encode('utf-8')).hexdigest()[:10]}"
            )
            nodes.append(
                {
                    "id": point_id,
                    ...
                    "order": point_order,
                    "number": point_number,
                    "title": build_point_title(point_number, point_body),
                    "content": point_content,
                    "plainText": point_content,
                    "source": {
                        "url": page.get("url", urljoin(BASE_URL, item.href)),
                        "path": item.href,
                        "fetchedAt": fetched_at,
...
def build_dataset(max_workers: int = 6) -> Dict[str, object]:
    ...
    instructions = [
        build_instruction(
            instruction_id="pte",
            title="ПТЭ",
            short_description="Правила технической эксплуатации железных дорог Российской Федерации",
        ...
        build_instruction(
            instruction_id="isi",
            title="ИСИ",
            short_description="Инструкция по сигнализации на железнодорожном транспорте Российской Федерации",
        ...
        build_instruction(
            ...
            title="ИДП",
            short_description=(
                "Инструкция по организации движения поездов и маневровой работы "
    ...
    return {
        ...
        "source": {
            ...
            "tocUrl": TOC_URL,
            "generatedAt": now,
            "notes": "Полные тексты собраны с открытых страниц нормативного документа.",
        },
