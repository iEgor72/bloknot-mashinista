    ...
    (function initStandaloneScrollFreezeFix() {
      ...
      function patchContainer(el) {
        ...
        el.addEventListener('touchend', function() {
          ...
          var wasMoving = lastMoveTs > 0 && (Date.now() - lastMoveTs) < 120;
          if (dy < 12 || !wasMoving) return;

          var scrollAtEnd = el.scrollTop;
          var atTop = scrollAtEnd <= 1;
          var atBottom = scrollAtEnd + el.clientHeight >= el.scrollHeight - 2;
      ...
      if (APP_CONTENT) patchContainer(APP_CONTENT);
      patchContainer(document.querySelector('.shift-detail-body'));
      patchContainer(document.querySelector('.shift-detail-scroll'));
      var sheets = document.querySelectorAll('.bottom-sheet');
      for (var si = 0; si < sheets.length; si++) patchContainer(sheets[si]);
    })();
