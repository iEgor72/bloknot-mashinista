...
async function runOfflineReloadCheck() {
  ...
  const mixedCacheSeed = await page.evaluate(async () => {
    ...
    const liveResponse = await fetch('/scripts/v396/time-utils.js?mixed-cache-source=' + Date.now(), { cache: 'no-store' });
    ...
    await staleCache.put('/scripts/v395/time-utils.js', new Response(staleSource, {
      status: 200,
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      ...
      await currentCache.put('/scripts/v396/time-utils.js', new Response(liveSource, {
        status: 200,
        headers: { 'Content-Type': 'application/javascript; charset=utf-8' },
      }));
      const currentKeys = await currentCache.keys();
      runtimeKeys = currentKeys.filter((request) => new URL(request.url).pathname === '/scripts/v396/time-utils.js');
  ...
  const repairNavigation = page.waitForURL((url) => (
    url.searchParams.get('runtime_repair') === 'v396' && Boolean(url.searchParams.get('repair_nonce'))
  ), { timeout: uiTimeoutMs });
...
async function runStaleOfflineRuntimeCheck() {
  ...
  await page.route('**/scripts/v396/time-utils.js*', (route) => route.fulfill({
    status: 200,
...
async function runBootFallbackCheck() {
  ...
  await page.route('**/scripts/v396/auth.js*', (route) => route.abort());
  await page.route('**/scripts/v396/app.js*', (route) => route.abort());
...
async function main() {
  ...
  const unexpectedRequestFailures = report.requestFailures.filter((failure) => {
    if (failure.label === 'boot-fallback' && /\/scripts\/v396\/(auth|app)\.js/.test(failure.url)) return false;
