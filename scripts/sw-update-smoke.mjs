...
const bootstrapSource = await readFile(path.join(root, 'sw-bootstrap-v389.js'), 'utf8');
...
function createHarness(source, initialController) {
  ...
  const window = {
    __SHIFT_TRACKER_SW_URL: '/sw.js?v=v389',
    navigator: {},
    sessionStorage,
    location: {
      reload() {
...
firstInstall.serviceWorker.controller = { scriptURL: '/sw.js?v=v389' };
...
upgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v389' };
...
currentRegisterUpgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v389' };
...
assert.match(indexSource, /<script src="\/sw-bootstrap-v389\.js" defer><\/script>/);

console.log('Service-worker v388→v389 coherent-runtime upgrade smoke passed.');
