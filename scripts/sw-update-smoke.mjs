...
const bootstrapSource = await readFile(path.join(root, 'sw-bootstrap-v412.js'), 'utf8');
...
function createHarness(source, initialController) {
  ...
  const window = {
    __SHIFT_TRACKER_SW_URL: '/sw.js?v=v412',
    navigator: {},
    sessionStorage,
    location: {
      reload() {
...
firstInstall.serviceWorker.controller = { scriptURL: '/sw.js?v=v412' };
...
upgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v412' };
...
currentRegisterUpgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v412' };
...
assert.match(workerSource, /'\/scripts\/v412\/poekhali-tracker\.js'/);
assert.match(workerSource, /'\/scripts\/v412\/poekhali-station-names\.js'/);
assert.match(workerSource, /'\/scripts\/v412\/poekhali-backup\.js'/);
assert.doesNotMatch(workerSource, /'\/scripts\/v412\/partners\.js'/);
assert.match(workerSource, /'\/scripts\/v412\/shift-form\.js'/);
assert.match(workerSource, /'\/scripts\/v412\/vu45-calculator\.js'/);
assert.match(workerSource, /'\/scripts\/v412\/vu45-ui\.js'/);
...
assert.match(indexSource, /href="\/styles\/v412\/56-profile\.css"/);
assert.match(indexSource, /href="\/styles\/v412\/58-vu45\.css"/);
assert.doesNotMatch(indexSource, /href="\/styles\/(?!v412\/)/);
assert.match(workerSource, /'\/styles\/v412\/56-profile\.css'/);
assert.match(indexSource, /src="\/scripts\/v412\/render\.js"/);
assert.match(indexSource, /src="\/scripts\/v412\/poekhali-station-names\.js"/);
assert.doesNotMatch(indexSource, /src="\/scripts\/(?!v412\/)/);
assert.match(workerSource, /'\/scripts\/v412\/render\.js'/);

console.log('Service-worker v405→v412 coherent-runtime and versioned shell namespace smoke passed.');
