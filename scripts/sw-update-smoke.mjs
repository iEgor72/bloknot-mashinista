...
const bootstrapSource = await readFile(path.join(root, 'sw-bootstrap-v408.js'), 'utf8');
...
function createHarness(source, initialController) {
  ...
  const window = {
    __SHIFT_TRACKER_SW_URL: '/sw.js?v=v408',
    navigator: {},
    sessionStorage,
    location: {
      reload() {
...
firstInstall.serviceWorker.controller = { scriptURL: '/sw.js?v=v408' };
...
upgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v408' };
...
currentRegisterUpgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v408' };
...
assert.match(workerSource, /'\/scripts\/v408\/poekhali-tracker\.js'/);
assert.match(workerSource, /'\/scripts\/v408\/poekhali-station-names\.js'/);
assert.match(workerSource, /'\/scripts\/v408\/poekhali-backup\.js'/);
assert.doesNotMatch(workerSource, /'\/scripts\/v408\/partners\.js'/);
assert.match(workerSource, /'\/scripts\/v408\/shift-form\.js'/);
...
assert.match(indexSource, /<script src="\/sw-bootstrap-v408\.js" defer><\/script>/);

assert.match(indexSource, /href="\/styles\/v408\/56-profile\.css"/);
assert.doesNotMatch(indexSource, /href="\/styles\/(?!v408\/)/);
assert.match(workerSource, /'\/styles\/v408\/56-profile\.css'/);
assert.match(indexSource, /src="\/scripts\/v408\/render\.js"/);
assert.match(indexSource, /src="\/scripts\/v408\/poekhali-station-names\.js"/);
assert.doesNotMatch(indexSource, /src="\/scripts\/(?!v408\/)/);
assert.match(workerSource, /'\/scripts\/v408\/render\.js'/);

console.log('Service-worker v405→v408 coherent-runtime and versioned shell namespace smoke passed.');
