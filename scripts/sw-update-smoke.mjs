...
const bootstrapSource = await readFile(path.join(root, 'sw-bootstrap-v387.js'), 'utf8');
...
function createHarness(source, initialController) {
  ...
  const window = {
    __SHIFT_TRACKER_SW_URL: '/sw.js?v=v387',
    navigator: {},
    sessionStorage,
    location: {
      reload() {
...
firstInstall.serviceWorker.controller = { scriptURL: '/sw.js?v=v387' };
...
const upgrade = createHarness(bootstrapSource, { scriptURL: '/sw.js?v=v386' });
assert.equal(typeof upgrade.listeners.controllerchange, 'function');
upgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v387' };
upgrade.listeners.controllerchange();
upgrade.listeners.controllerchange();
assert.equal(upgrade.reloads, 1, 'controller upgrade must reload only once');

const currentRegisterUpgrade = createHarness(registerSource, { scriptURL: '/sw.js?v=v386' });
currentRegisterUpgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v387' };
...
assert.match(indexSource, /<script src="\/sw-bootstrap-v387\.js" defer><\/script>/);

console.log('Service-worker v386→v387 coherent-runtime upgrade smoke passed.');
