...
const bootstrapSource = await readFile(path.join(root, 'sw-bootstrap-v393.js'), 'utf8');
...
function createHarness(source, initialController) {
  ...
  const window = {
    __SHIFT_TRACKER_SW_URL: '/sw.js?v=v393',
    navigator: {},
    sessionStorage,
    location: {
      reload() {
...
firstInstall.serviceWorker.controller = { scriptURL: '/sw.js?v=v393' };
...
upgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v393' };
...
currentRegisterUpgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v393' };
...
assert.match(indexSource, /<script src="\/sw-bootstrap-v393\.js" defer><\/script>/);

console.log('Service-worker v392→v393 coherent-runtime upgrade smoke passed.');
