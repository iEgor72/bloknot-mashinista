...
const bootstrapSource = await readFile(path.join(root, 'sw-bootstrap-v407.js'), 'utf8');
...
function createHarness(source, initialController) {
  ...
  const window = {
    __SHIFT_TRACKER_SW_URL: '/sw.js?v=v407',
    navigator: {},
    sessionStorage,
    location: {
      reload() {
...
firstInstall.serviceWorker.controller = { scriptURL: '/sw.js?v=v407' };
...
upgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v407' };
...
currentRegisterUpgrade.serviceWorker.controller = { scriptURL: '/sw.js?v=v407' };
...
assert.match(workerSource, /'\/scripts\/v407\/poekhali-tracker\.js'/);
assert.match(workerSource, /'\/scripts\/v407\/poekhali-station-names\.js'/);
assert.match(workerSource, /'\/scripts\/v407\/poekhali-backup\.js'/);
assert.doesNotMatch(workerSource, /'\/scripts\/v407\/partners\.js'/);
assert.match(workerSource, /'\/scripts\/v407\/shift-form\.js'/);
...
assert.match(indexSource, /<script src="\/sw-bootstrap-v407\.js" defer><\/script>/);

assert.match(indexSource, /href="\/styles\/v407\/56-profile\.css"/);
assert.doesNotMatch(indexSource, /href="\/styles\/(?!v407\/)/);
assert.match(workerSource, /'\/styles\/v407\/56-profile\.css'/);
assert.match(indexSource, /src="\/scripts\/v407\/render\.js"/);
assert.match(indexSource, /src="\/scripts\/v407\/poekhali-station-names\.js"/);
assert.doesNotMatch(indexSource, /src="\/scripts\/(?!v407\/)/);
assert.match(workerSource, /'\/scripts\/v407\/render\.js'/);

console.log('Service-worker v405→v407 coherent-runtime and versioned shell namespace smoke passed.');
